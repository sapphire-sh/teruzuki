...
namespace teruzuki.Twitter
{
	public class OAuth
	{
		...
		public OAuth () : this (new Token ())
		{
		}

		public OAuth (Token token)
		{
			this.parameters = new Dictionary<string, string> ();
			this.OAuthCallback = "oob";
			this.OAuthConsumerKey = Constants.Token.CONSUMER_KEY;
			this.OAuthConsumerSecret = Constants.Token.CONSUMER_SECRET;
			this.OAuthTimestamp = GenerateTimestamp ();
			this.OAuthNonce = GenerateNonce ();
			this.OAuthSignatureMethod = "HMAC-SHA1";
			this.OAuthSignature = "";
			this.OAuthToken = token.AccessToken;
			this.OAuthTokenSecret = token.AccessTokenSecret;
